...
Original file is located at
    https://colab.research.google.com/drive/1ObrIGgheeLSoGfC8XmojQN64xYaqlabh
...
from sklearn.model_selection import train_test_split,cross_val_score  #for splitting the data  into training and test data
from sklearn.linear_model import LogisticRegression                      #predictive analysis algo based on probability 
from sklearn.ensemble import RandomForestClassifier   #random forest is an ensemble model. (Decision Tree Ensemble model)
from sklearn.metrics import accuracy_score,confusion_matrix, classification_report #accuracy score to show how well our model performs
import warnings                                    #to ignore any warnings that may occur
...
"""Step 6(I): Training The Model : Using Random Forest Classifier. 
...
"""Step 6(II): Training The Model : Using Logistic Regression

Logistic regression is a machine algorithm which is used for the classification problems, it is a predictive analysis algorithm and based on the concept of probability.
"""

LR = LogisticRegression().fit(X_train,Y_train)        #Using the imported Logistic Regression. X_train contains the data, Y_train contains the label (quality values).

"""Step 7 (I) : Model Evaluation (Random Forest Classifier)
...
cm1=confusion_matrix(Y_test, X_test_prediction)                   #Confusion Matrix
ax1 = sns.heatmap(cm1,annot=True, annot_kws={"fontsize":18})         
ax1.set(xlabel='predict', ylabel='true')                           
plt.show()

"""Step 7 (II) : Model Evaluation (Logistic Regression)"""

pred_LR = LR.predict(X_test)                    

cm2 = confusion_matrix(Y_test,pred_LR)                      #Confusion Matrix 
ax2 = sns.heatmap(cm2,annot=True,annot_kws={"fontsize":18})
ax2.set(xlabel='predict', ylabel='true')

pred2 = accuracy_score(Y_test,pred_LR)                       #Printing Accuracy Score
                                                             #Any accuracy over 80 (>0.80) is a very good model   
